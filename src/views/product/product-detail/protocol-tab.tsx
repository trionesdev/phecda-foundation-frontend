...
const ProtocolTab: FC<ProtocolTabProps> = ({
                                               product
                                           }) => {
    ...
    const handleSave = () => {
        form.validateFields().then((values) => {
            deviceApi.updateProductProtocolProperties(product.id, values).then(() => {
    ...
    const tabToolbar = <PageHeader title={`设备连接协议`} backIcon={false}
                                   extra={[<Button type={`primary`} onClick={handleSave}>保存</Button>]}/>
    return <VPanel className={styles.protocolTab} header={tabToolbar}>
        <div className={styles.protocolTabContentWrapper}>
            <Form form={form}>
                <Form.List name={`protocolProperties`}>
                    {(fields, {add, remove}) => (<>
                        {fields.map(({key, name}) => (<Space key={key} align={`baseline`}>
                            <Form.Item name={[name, 'label']} label={`名称`} rules={[{required: true}]}>
                                <Input/>
                            </Form.Item>
                            <Form.Item name={[name, 'name']} label={`属性`} rules={[{required: true}]}>
                                <Input/>
                            </Form.Item>
