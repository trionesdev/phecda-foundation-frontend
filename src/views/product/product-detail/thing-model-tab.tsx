...
const ThingModelTab: FC<ThingsModelTabProps> = ({product}) => {
    ...
    const handleQueryThingModel = () => {
        setLoading(true)
        deviceApi.queryThingModel(product.id).then((res: any) => {
    ...
    const alert = <div className={styles.thingsModelTabPrompt}>当前展示的是已发布到线上的功能定义，如需修改，<Link
        to={RoutesConstants.PRODUCT_THINGS_MODEL_DRAFT.path(product?.id)}>请点击</Link></div>
    return <VPanel className={styles.thingsModelTab} header={alert}>
        <GridTable style={{backgroundColor: "white"}} size={`small`}
                   columns={columns}
