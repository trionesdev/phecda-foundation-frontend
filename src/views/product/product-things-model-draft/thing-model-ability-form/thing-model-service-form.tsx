...
const ThingModelServiceForm: FC<ThingsModelServiceFormProps> = () => {
    ...
    return <>
        <Form.Item label={`功能名称`} name={[ability, `name`]} required={true}>
            <Input/>
        </Form.Item>
        <Form.Item label={`标识符`} name={[ability, `identifier`]} required={true}>
            <Input/>
        </Form.Item>
        <Form.Item label={`调用方式`} name={[ability, 'callType']} required={true} initialValue={`ASYNC`}>
            <Radio.Group>
        ...
        <Form.Item label={`描述`} name={[ability, `description`]} initialValue={''}>
            <Input.TextArea/>
        </Form.Item>
    </>
}
